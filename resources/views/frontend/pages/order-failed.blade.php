@extends('frontend.layouts.master')

@section('title', __('frontend.order_status_title'))

@section('main-content')

<x-breadcrumb
    :title="__('frontend.order_status')"
    :items="[
        ['label' => __('frontend.home'), 'url' => route('home')],
        ['label' => __('frontend.order_status')],
    ]" />

<section class="os-section">
    <div class="container">
        <div class="os-card" data-aos="fade-up" data-aos-duration="1000">

            <div class="os-icon os-icon-failed">
                <i class="fa-solid fa-xmark"></i>
            </div>

            <h2 class="os-title">{{ __('frontend.payment_unsuccessful') }}</h2>
            <p class="os-sub">{{ __('frontend.payment_failure_message') }}</p>

            <div class="os-panel">
                <h5>{{ __('frontend.payment_error') }}</h5>
                <p>{{ __('frontend.what_you_can_do') }}</p>
                <ul class="os-list mt-2">
                    <li>{{ __('frontend.check_payment_details') }}</li>
                    <li>{{ __('frontend.contact_bank') }}</li>
                    <li>{{ __('frontend.try_different_payment') }}</li>
                </ul>
            </div>

            <div class="os-panel">
                <h5>{{ __('frontend.need_assistance') }}</h5>
                <p>
                    {{ __('frontend.reach_out') }}. {{ __('frontend.we_are_here') }}
                    <a href="mailto:{{ env('APP_EMAIL') }}">{{ env('APP_EMAIL') }}</a>
                </p>
            </div>

            <div class="os-actions">
                <a href="{{ route('cart') }}" class="ct-submit">
                    <i class="fa-solid fa-rotate-right"></i>
                    <span>{{ __('frontend.try_again') }}</span>
                </a>
                <a href="{{ route('contact') }}" class="ct-submit os-btn-outline">
                    <span>{{ __('frontend.contact_us') }}</span>
                </a>
            </div>
        </div>
    </div>
</section>

@endsection
